...
module.exports = {
     ...
     * @return {String}
     */
    convert({ string, hrefConvert, dialect }) {
        string = string
            .replace(/\\/g, "\\\\")
            .replace(/ _/g, "&#95;")
            .replace(/_ /g, "&#95;")
         ...
         * @type {String}
         */
        let html = replaceExtras(replacements.reduce((string, replacement) => {
            return string.replace(replacement[0], replacement[1]);
        }, string));
        /**
         * looping through registered formatters
         */
        for (let _i = 0, formatters_1 = formatters; _i < formatters_1.length; _i++) {
            const formatter = formatters_1[_i];
            if (typeof formatter === "function")
                html = formatter({ doc: html, dialect });
        }
        html = html
            .replace(/<a.*?href="(.*?)".*?>(.*?)<\/a>/gi, (match, href, name) => {
