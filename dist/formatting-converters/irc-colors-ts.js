...
function ColorifyText({ side = "fg", text, color }) {
...
function MoodifyText({ text, colors, mood }) {
    if (mood === "none")
        return text;
    if (mood === "mood") {
...
function stripStyle(str) {
    ...
    for (let i = 0, len = str.length; i < len; i++) {
        ...
        if (styleChars[char] || char === globalC) {
            ...
            if (lastChar && lastChar[0] === char) {
                let p0 = parseInt(lastChar[1].toString());
    ...
    for (let char of path.reverse()) {
        if (char[0] !== globalC) {
            let pos = parseInt(char[1].toString());
