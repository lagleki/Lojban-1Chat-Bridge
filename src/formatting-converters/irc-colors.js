...
const custom = {
  ...
  mood: (str, colorArr, mode) => {
    ...
    hash = Math.abs(hash % mood.length);

    let l = mood[hash][1].length;
    let i = 0;
    ...
    return (
      str
        ...
        .map(
          c =>
            c !== " "
              ? exports[mood[hash][0]](exports[mood[hash][1][i++ % l]](c))
              : c
        )
        // .map(c => (c !== " " ? exports[mood[hash][1][i++ % l]](c) : c))
        .join("")
    );
  }
};
...
exports.nickcolor = (str, { nickcolor, palette_for_nickcolor }, mode) => {
  if (str.length === 0 || !nickcolor) return str;
  return exports["mood"](str, palette_for_nickcolor, mode);
};
