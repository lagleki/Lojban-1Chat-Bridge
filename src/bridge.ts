		...
		if (config.discord.guildId) {
			...
			if (guild)
				generic.discord.guilds = [
					guild,
					...generic.discord.guilds.filter((_guild: any) => _guild.id !== guild.id),
				];
		}
		generic.discord.guilds.getAll = (name: string) =>
			[].concat(...generic.discord.guilds.map((guild: any) => guild[name].array()));
		resolve();
	});
};
				...
				(error: any, response: any, body: any) => {
					if (err) {
						...
						const team = JSON.parse(body).find((i: any) => {
							return i.display_name === config.mattermost.team || i.name === config.mattermost.team;
						});
...
async function FormatMessageChunkForSending({
	...
	if (quotation) {
		...
		chunk = generic.LocalizeString({
			...
			arrElemsToInterpolate: [['author', author], ['chunk', chunk], ['title', title]],
		});
...
sendTo.facebook = async ({ channelId, author, chunk, action, quotation, file }: IsendToArgs) => {
	if (R.path(['channelMapping', 'facebook', channelId, 'settings', 'readonly'], config) || !generic.facebook.client)
		return;
...
sendTo.telegram = async ({ channelId, author, chunk, action, quotation, file }: IsendToArgs) => {
	if (R.path(['channelMapping', 'telegram', channelId, 'settings', 'readonly'], config)) return;
...
sendTo.discord = async ({ channelId, author, chunk, action, quotation, file }: IsendToArgs) => {
	if (R.path(['channelMapping', 'discord', channelId, 'settings', 'readonly'], config)) return;
...
sendTo.mattermost = async ({ channelId, author, chunk, action, quotation, file }: IsendToArgs) => {
	if (R.path(['channelMapping', 'mattermost', channelId, 'settings', 'readonly'], config)) return;
...
sendTo.vkwall = async ({ channelId, author, chunk, action, quotation, file }: IsendToArgs) => {
	if (R.path(['channelMapping', 'vkwall', channelId, 'settings', 'readonly'], config)) return;
...
sendTo.vkboard = async ({ channelId, author, chunk, action, quotation, file }: IsendToArgs) => {
	if (
		R.path(['channelMapping', 'vkboard', channelId, 'settings', 'readonly'], config) //todo: !vk.WaitingForCaptcha
	)
		return;
...
sendTo.slack = async ({ channelId, author, chunk, action, quotation, file }: IsendToArgs) => {
	if (R.path(['channelMapping', 'slack', channelId, 'settings', 'readonly'], config)) return;
...
sendTo.irc = async ({ channelId, author, chunk, action, quotation, file }: IsendToArgs) => {
	if (R.path(['channelMapping', 'irc', channelId, 'settings', 'readonly'], config)) return;
...
prepareToWhom.irc = function({ text, channelId }: { text: string; channelId: string | number }) {
	const ColorificationMode = R.pathOr('color', ['channelMapping', 'irc', channelId, 'settings', 'nickcolor'], config);
...
prepareToWhom.fallback = function({ text, channelId }: { text: string; channelId: string | number }) {
	return `${text}: `;
};
prepareAuthor.irc = function({ text, channelId }: { text: string; channelId: string | number }) {
	const ColorificationMode = R.pathOr('color', ['channelMapping', 'irc', channelId, 'settings', 'nickcolor'], config);
	return `${ircolors.MoodifyText({
		text,
		mood: ColorificationMode,
...
prepareAuthor.fallback = function({ text, channelId }: { text: string; channelId: string | number }) {
	return `${text}`;
};
...
async function sendFrom({
	...
	if (!ConfigNode) return generic.LogToAdmin(`error finding assignment to ${messenger} channel with id ${channelId}`);
	if (!text || text === '') return;
	text = await convertFrom[messenger](text);
	text = text.replace(/^(<br\/>)+/, '');
	for (const messengerTo of Object.keys(config.channelMapping)) {
		if (config.MessengersAvailable[messengerTo] && ConfigNode[messengerTo] && messenger !== messengerTo) {
			...
			debug('generic')(`converting for messenger ${messengerTo} the text "` + text + `"`);
			let textTo = await convertTo[messengerTo](text);
			debug('generic')(`converted for messenger ${messengerTo} to text "` + textTo + `"`);
...
receivedFrom.discord = async (message: any) => {
	if (!R.path(['channelMapping', 'discord', R.pathOr('', ['channel', 'id'], message).toString()], config)) return;
...
receivedFrom.facebook = async (message: any) => {
	if (!R.path(['channelMapping', 'facebook', (message.threadId || '').toString()], config)) return;
	let err, res;
	...
	if (message.stickerId) message.attachments.push({ id: message.stickerId, type: 'sticker' });
...
receivedFrom.telegram = async (message: Telegram.Message) => {
	...
	if (!config.channelMapping.telegram[message.chat.id]) {
		if (config.cache.telegram[message.chat.title] && config.cache.telegram[message.chat.title] === message.chat.id)
...
generic.discord.reconstructPlainText = (message: any, text: string) => {
	...
	if (massMentions.some((massMention: string) => text.includes(massMention)) && !config.discord.massMentions) {
	...
	if (matches && matches[0])
		for (let match of matches) {
			...
			const member = message.channel.guild.members
				.array()
				.find((member: any) => member.user.username && member.user.id.toLowerCase() === core);
...
async function sendFromTelegram({ message, quotation }: { message: any; quotation?: boolean }) {
	if (!message) return;
	let action;
	...
	const reply_to_bot = quotation && message.from.id === config.telegram.myUser.id ? true : false;
	let author = '';
	...
	for (let i: number = 0; i < arrMessage.length; i++) {
		...
		if (el === 'text') {
			...
			if (quotation && jsonMessage[el].text.length > config['telegram'].MessageLength)
				jsonMessage[el].text = `${jsonMessage[el].text.substring(0, config['telegram'].MessageLength - 1)} ...`;
		}
		if (jsonMessage[el].url)
			[jsonMessage[el].url, jsonMessage[el].local_file] = await generic.telegram.serveFile(jsonMessage[el].url);
		const arrForLocal = Object.keys(jsonMessage[el]).map(i => [i, jsonMessage[el][i]]);
...
receivedFrom.vkwall = async (message: any) => {
	...
	if (!config.channelMapping.vkwall[channelId] || '-' + config.vkwall.group_id === message.from_id.toString()) return;
	if (!generic.vkwall.client.app) {
		config.MessengersAvailable.vkwall = false;
		return;
	}
	...
	text.replace(/\[[^\]]+:bp-([^\]]+)_([^\]]+)\|[^\]]*\]/g, (match: any, group_id: string, post_id: string) => {
...
receivedFrom.vkboard = async (message: any) => {
	...
	if (!config.channelMapping.vkboard[channelId] || message.topic_owner_id === message.from_id) return;
	if (!generic.vkboard.client.app) {
		config.MessengersAvailable.vkboard = false;
		return;
	}
...
receivedFrom.slack = async (message: any) => {
	...
	if (
		(message.subtype && !['me_message', 'channel_topic', 'message_changed'].includes(message.subtype)) ||
	...
	if (message.subtype === 'channel_topic' && message.topic && message.topic !== '') {
		action = 'topic';
...
receivedFrom.mattermost = async (message: any) => {
	...
	if (R.path(['event'], message) === 'post_edited') {
		...
		if (R.path(['data', 'team_id'], message) !== config.mattermost.team_id) return;
...
receivedFrom.irc = async ({
	...
	if (type === 'message') {
		...
		text = `<${ircolors.stripColorsAndStyle(author).replace(/_+$/g, '')}>: ${text}`
			.replace(/^<[^ <>]+?>: <([^<>]+?)> ?: /, '*$1*: ')
			.replace(/^<[^ <>]+?>: &lt;([^<>]+?)&gt; ?: /, '*$1*: ')
			.replace(/^<([^<>]+?)>: /, '*$1*: ')
			.replace(/^\*([^<>]+?)\*: /, '<b>$1</b>: ');
...
convertFrom.telegram = async (text: string) => marked.parser(lexer.lex(generic.escapeHTML(text)));
convertFrom.vkboard = async (text: string) => generic.escapeHTML(text).replace(/\[[^\]]*\|(.*?)\](, ?)?/g, '');
...
convertFrom.slack = async (text: string) => {
	...
	const safeMatch = (match: RegExpExecArray | null, tag: string, trigger?: string) => {
		...
		if (!postfix_ok) {
			const charAtRight: string = match.input.substr(match.index + match[0].length, 1);
	...
	const matchBold = (match: RegExpExecArray | null) => safeMatch(match, tag('strong', payloads(match[1])), '*');

	const matchItalic = (match: RegExpExecArray | null) => safeMatch(match, tag('em', payloads(match[1])), '_');

	const matchFixed = (match: RegExpExecArray | null) => safeMatch(match, tag('code', payloads(match[1])));

	const notAlphanumeric = (input: string) => !RE_ALPHANUMERIC.test(input);

	const notRepeatedChar = (trigger: string, input: string) => !trigger || trigger !== input;

	async function parseSlackText(text: string) {
		const jsonChannels: Json = {};
		const jsonUsers: Json = {};
		text.replace(/<#(C\w+)\|?(\w+)?>/g, (match: any, channelId: any, readable: any) => {
			jsonChannels[channelId] = channelId;
			return channelId;
		});
		text.replace(/<@(U\w+)\|?(\w+)?>/g, (match: any, userId: any, readable: any) => {
		...
		return (
			emoji
				...
				.replace(/<#(C\w+)\|?(\w+)?>/g, (match: any, channelId: any, readable: any) => {
					return `#${readable || jsonChannels[channelId]}`;
				})
				.replace(/<@(U\w+)\|?(\w+)?>/g, (match: any, userId: any, readable: any) => {
					return `@${readable || jsonUsers[userId]}`;
				})
				.replace(/<(?!!)([^|]+?)>/g, (match: any, link: any) => link)
				.replace(/<!(\w+)\|?(\w+)?>/g, (match: any, command: any, label: any) => `<${label || command}>`)
...
convertFrom.mattermost = async (text: string) => marked.parser(lexer.lex(generic.escapeHTML(text)));
convertFrom.discord = async (text: string) => marked.parser(lexer.lex(generic.escapeHTML(text)));
...
convertTo['mattermost'] = async (text: string) => html2md.convert({ string: text }); // .replace(/\*/g, "&#42;").replace(/\_/g, "&#95;")
convertTo['discord'] = async (text: string) =>
	await generic.unescapeHTML(html2md.convert({ string: text, hrefConvert: false }), true);
	...
	if (IsSpam(cloned_message)) {
		if (message.text && message.text.search(/\bt\.me\b/) >= 0) {
			...
			if (!err) {
				...
				if (IsSpam(cloned_message)) generic.telegram.DeleteMessage({ message, log: true });
			} else {
				generic.LogToAdmin(
			...
			if (!err) {
				...
				generic.LogToAdmin(
					`error ${err} on getting an invite link of the chat ${cloned_message.chat.id} ${
		...
		const xovahe =
			arrText.filter(i => lojban.ilmentufa_off("lo'u " + i + " le'u").tcini === 'snada').length / arrText.length;
		if (xovahe < 0.5) {
			generic.telegram.client
				.sendMessage(
					message.chat.id,
	...
	if (config.telegram.remove_added_bots)
		R.pathOr([], ['new_chat_members'], message).map((u: Telegram.User) => {
			if (u.is_bot && R.path(['telegram', 'myUser', 'id'], config) !== u.id)
				generic.telegram.client.kickChatMember(message.chat.id, u.id).catch(catchError);
		});
}
...
function TelegramRemoveNewMemberMessage(message: Telegram.Message) {
	if (
		...
		R.pathOr([], ['new_chat_members'], message).filter(
			(u: Telegram.User) =>
				(u.username || '').length > 100 || (u.first_name || '').length > 100 || (u.last_name || '').length > 100
		).length > 0
	) {
...
async function TelegramLeaveChatIfNotAdmin(message: Telegram.Message) {
	if (!R.path(['chat', 'id'], message) || !R.path(['telegram', 'myUser', 'id'], config)) return;
...
generic.telegram.DeleteMessage = async ({ message, log }: { message: Telegram.Message; log: boolean }) => {
...
NewChannelAppeared.telegram = async ({ channelName, channelId }: { channelName: string; channelId: string }) => {
	...
	if (err) generic.LogToAdmin(`got problem in the new telegram chat ${channelName}, ${channelId}`);
	if (err) {
		console.error(err);
		return;
	}
...
GetChannels.telegram = async () => {
	...
	let [err, res] = await to(
		new Promise(resolve => {
			resolve(JSON.parse(fs.readFileSync(`${process.env.HOME}/.${package_json.name}/cache.json`)).telegram);
		})
	);
	if (err || !res) res = {};
...
GetChannels.mattermost = async () => {
	...
	url = `${config.mattermost.ProviderUrl}/api/v4/users/${config.mattermost.user_id}/teams/${
...
async function PopulateChannelMappingCore({ messenger }: { messenger: string }) {
	...
	config.channels.map((i: any) => {
		...
		if (config.cache[messenger]) i_mapped = R.path(['cache', messenger, i[messenger]], config);
		if (!i_mapped) return;
		const mapping: any = {
			settings: {
		...
		for (const key of arrMappingKeys) mapping[key] = R.pathOr(i[key], ['cache', key, i[key]], config);
...
generic.MessengersAvailable = () => {
	...
	if (R.pathOr('', ['facebook', 'email'], config) === '' || R.pathOr('', ['facebook', 'password'], config) === '')
	...
	if (R.pathOr('', ['telegram', 'token'], config) === '') config.MessengersAvailable.telegram = false;
	if (
...
// helper functions
generic.sendOnlineUsersTo = ({ network, channel }: { network: string; channel: string }) => {
	...
	if (network === 'telegram') {
		...
		generic.telegram.client.sendMessage(objChannel.id, strNames).catch((e: any) => console.log(e.toString()));
	}
};
...
generic.LogMessageToAdmin = async (message: Telegram.Message) => {
	if (config.telegram.admins_userid)
		await to(
			generic.telegram.client.forwardMessage(config.telegram.admins_userid, message.chat.id, message.message_id)
		);
};
...
generic.downloadFile = async ({
	...
	[err, res] = await to(
		new Promise(resolve => {
			const newname = `${local_path}/${randomStringName}${path.extname(local_fullname)}`;
...
generic.LocalizeString = ({
	...
	try {
		const language = R.pathOr('English', ['channelMapping', messenger, channelId, 'settings', 'language'], config);
