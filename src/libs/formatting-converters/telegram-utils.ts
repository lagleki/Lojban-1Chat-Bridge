...
const rewriteTextAtPosition = (
  ...
  text.substr(0, position) + rewriteText + text.substr(position + rewriteLength)
...
const escapeLinkChars = (text: string) =>
  escapeCommonChars(escapeChars(text, [")", "\\"]))
...
const wrapTextWithMarkdownEntity = (
  text: string,
...
export const fillMarkdownEntitiesMarkup = (
  text: string,
  ...
  const entitiesChunks = R.groupBy((entity: MessageEntity) => entity.offset)(
    entities,
  )
