...
function stripColors(str: string) {
  // IRC mIRC color / bold control bytes
  // eslint-disable-next-line no-control-regex -- intentional IRC protocol bytes
...
function stripStyle(str: string) {
  ...
  for (let i = 0, len = str.length; i < len; i++) {
    ...
    if (styleChars[char] || char === globalC) {
      ...
      if (lastChar && lastChar[0] === char) {
        const p0: number = parseInt(lastChar[1].toString())
  ...
  for (const char of path.reverse()) {
    if (char[0] !== globalC) {
      const pos: number = parseInt(char[1].toString())
